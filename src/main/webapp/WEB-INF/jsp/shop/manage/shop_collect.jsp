        ...
        <div class="main">
          <div class="datas_detail pull-left">
            <ul class="tabs clearfix">
              <li><a href="${host}/shop/center" title="店铺商品">店内首页</a></li> 
              ...
              <li class="cur"><a href="${pageContext.request.contextPath}/shop/admin/collect">我的收藏</a></li>
            ...
            <div class="items">
              ...
              <c:forEach  items="${collectList }" var="collect">
				...
				<div class="card clearfix">
					...
					<img src="${pageContext.request.contextPath}/shop/images/logo/zip.svg"></a>
					</div>			 
										  
						<div class="content">
							<h3>
								<a target="_blank" href="${host}/shop/source/${collect.source.id }">${collect.source.name }</a>
							</h3>
							<p class="brief" style="display:inline-block;">${collect.source.description }</p>
							<p class="tags clearfix">
																					<a target="_blank" href="/tag/bootstrap">${collect.source.tag }</a>
																				</p>
							<div class="times">
								<label>下载次数：</label>${collect.source.downNum }						</div>
							<div class="date">
								<label>上传时间：</label><fmt:formatDate value="${collect.source.upload_time}" pattern="yyyy-MM-dd HH:mm "/>						</div>
							<div class="score">
								<label>所需积分：</label>${collect.source.douNum }					
							</div>
							<div class="flag">
             					<a href="javascript:void(0);" onclick="cancel_fav('${collect.source.name }','${collect.source.id}')">取消收藏</a>
             				</div>
...
<script type="text/javascript">
function cancel_fav(sourceName,sourceId){
	
	//$.confirm("<p>您确定要退出sharehoo社区</p><p>一个人浪迹天涯吗？</P>");
	swal({
		title : "您确定要取消对该资源的关注吗？",
		text : '<span style="color:red">让它一个人浪迹天涯吗？</span>',
		type : "warning",
		html : true,
		showCancelButton : true,
		closeOnConfirm : false,
		confirmButtonText : "是的，忍心取消",
		confirmButtonColor : "#ec6c62"
	}, function() {
		$.post("${host}/shop/source/cancelFocus", {
			sourceId:sourceId
		}, function(result) {
			if (result.status == 200) {
				tipOk("取消收藏成功",function(){
