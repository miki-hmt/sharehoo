...
public class TopicServiceImpl implements TopicService {
    ...
    private static final String REGISTER_PREFIX = "lock:register:";
    private static final String TOPIC_PREFIX = "lock:";
    ...
    public void checkRegisterLimit() {
        ...
        Object registerCount = redisUtil.get(REGISTER_PREFIX + ip);
        long time = System.currentTimeMillis() + TIMEOUT;

        if (ObjectUtils.isEmpty(registerCount)) {
            //上锁
            if (!redisLock.lock(ip, String.valueOf(time))) {
                //获取锁失败，不执行操作
                throw new UserException(402, "注册过于频繁，请稍后重试...");
            }
            redisUtil.setEx(REGISTER_PREFIX + ip, 1, 1, TimeUnit.DAYS);
            return;
        }

        //获取发布的数量
        int count = Integer.parseInt(String.valueOf(registerCount));
        Long expire = redisUtil.getExpire(REGISTER_PREFIX + ip);
        //没过期，且发布数量超过5
        if (expire > -1 && count > 3) {
            throw new UserException(403, "今日ip注册数量已达上限，请明日重试...");
        }

        //上锁
        if (!redisLock.lock(ip, String.valueOf(time))) {
            //获取锁失败，不执行操作
            throw new UserException(402, "注册过于频繁，发布失败，请稍后重试...");
        }

        //更新注册数量
        redisUtil.setEx(REGISTER_PREFIX + ip, count + 1, 1, TimeUnit.DAYS);
        if (expire <= -1) {
            redisUtil.setEx(REGISTER_PREFIX + ip, 1, 1, TimeUnit.DAYS);
        }

        //释放锁
        redisLock.release(ip, String.valueOf(time));
        logger.info("####### 释放锁成功....");
    }
    ...
    public void checkPublishLimit() {
        ...
        Object publicCount = redisUtil.get(TOPIC_PREFIX + userId);
        long time = System.currentTimeMillis() + TIMEOUT;
        ...
        if (ObjectUtils.isEmpty(publicCount)) {
            ...
            redisUtil.set(TOPIC_PREFIX + userId, "1");
            return;
        }

        //获取发布的数量
        int count = Integer.parseInt(String.valueOf(publicCount));
        Long expire = redisUtil.getExpire(TOPIC_PREFIX + userId);
        //没过期，且发布数量超过5
        if (expire > -1 && count > 5) {
            throw new UserException(403, "今日发布数量已达上限，请明日重试...");
        }
        ...
        //更新发布数量
        redisUtil.setEx(TOPIC_PREFIX + userId, count + 1, 1, TimeUnit.DAYS);
        if (expire <= -1) {
            redisUtil.setEx(TOPIC_PREFIX + userId, 1, 1, TimeUnit.DAYS);
        }
    ...
    public List<Topic> findPtTopicListBySectionId(int sectionId, PageBean pageBean) {
        ...
        hql.append(" and top=0 and status = 1 order by modifyTime desc");
        if (pageBean != null) {
            return baseDAO.find(hql.toString().replaceFirst("and", "where"), param, pageBean);
        } else {
            return baseDAO.find(hql.toString().replaceFirst("and", "where"), param);
...
//					System.out.println("我在走缓存...."+json);
    ...
    public List<Topic> getNewTopicListBySection(int sectionId) {
        ...
        }    //获取redis中的数据，异常可以忽略	2019.08.19	miki
        ...
        if (sectionId == 21 || sectionId == 11 || sectionId == 1 || sectionId == 2 || sectionId == 3 || sectionId == 4 ||
            ...
            hql = "select topic.id,topic.title,topic.user.nickName,concat(topic.code,''),topic.user.id,topic.user.nickNameId from Topic as topic where topic.section.id=? and topic.status = 1 order by topic.publishTime desc";
        } else {
            hql = "from Topic as topic where topic.section.id=? and topic.status = 1 order by topic.publishTime desc";
        }
        param.add(sectionId);
    ...
    public List<Topic> getNewsList(int count) {
        // TODO Auto-generated method stub
        List<Object> param = new LinkedList<Object>();
        String hql = "from Topic as topic where topic.title not like '1' and topic.status = 1 order by topic.publishTime desc";
